...
## 16 de septiembre de 2026 — Primer módulo de métricas docentes

La docente pide saber quiénes ingresaron y qué actividades costaron más, y autoriza implementar lo que resulte necesario y útil. Se opta por una primera medición de **entregas del Módulo 3**, no de visitantes únicos: GitHub Pages no almacena el estado de cada navegador en un servidor propio y el curso usa almacenamiento local en computadoras compartidas. Registrar identidades o visitas individuales requeriría otra arquitectura y decisiones institucionales sobre datos de estudiantes. No se activa seguimiento automático ni se envían eventos a terceros.

Cada estudiante puede adjuntar a Classroom, junto al PDF, un JSON de métricas sin nombre, respuestas abiertas, código ni enlace. El tablero docente importa esos archivos en el navegador y muestra indicadores agregados: intentos, reparaciones resueltas, consultas de soluciones, criterios detectados, revisión, prueba entre pares y respuestas de salida. Exporta un CSV de totales para la bitácora. La fecha de exportación es metadato, no identificación. La asociación con estudiantes, si la docente la necesita para acompañarlos, permanece en Classroom y en los PDF, no en el tablero. El diseño evita llamar “dificultad” a un número de intentos sin más evidencia; debe contrastarse con producciones y observación. Es una propuesta técnica pendiente de prueba en clase, no un resultado de investigación.

La docente amplía el alcance a CursoContable. Se adapta el tablero para importar también la exportación sin nombres de ese curso, que registra intentos, pistas y resultados de ocho misiones. Los resúmenes y denominadores de cada curso se presentan por separado; el CSV indica el curso de cada indicador. Esta ampliación conserva las limitaciones anteriores: son entregas recibidas, no visitas ni personas únicas, y no se infieren resultados de aprendizaje sin contraste cualitativo. La bitácora específica de Contable registra su implementación.

### Rectificación del requisito docente: uso automático y difusión entre colegas

La docente aclara que puede no disponer de los PDF y que el alumnado no debe ver ni entregar métricas. No busca nombres: Classroom ya cubre la identificación y entrega. El interés es saber si los cursos se usan, dónde aparecen dificultades y si tienen uso más allá de sus estudiantes, al compartirlos con colegas. Por tanto, el tablero público y las exportaciones JSON implementados arriba **no satisfacen el requisito**; quedan pendientes de retiro o reemplazo. No se debe presentar su publicación como solución definitiva ni fabricar indicadores a partir de entregas ausentes.

Se propone medir de forma automática y agregada visitas, entrada y avance por módulo o misión, intentos, pistas o soluciones y finalización. Para diferenciar difusión, usar enlaces etiquetados para Classroom y para colegas, además de las fuentes de referencia, sin interpretar una visita como uso didáctico ni un enlace reenviado como prueba de identidad docente. La medición externa y el acceso privado requieren elegir un servicio, acordar condiciones institucionales y configurar el sitio; **no se activó seguimiento remoto** en esta fecha. La hipótesis a contrastar es si estas señales ayudan a priorizar cambios de formato y contenido.

La docente acota el alcance: **por ahora alcanzan las visitas**. También quiere distinguir el acceso procedente de sus clases del que deriva de compartir los cursos con colegas. Se retiran del prototipo el botón de exportación para estudiantes y el tablero público de archivos; no se toca la entrega PDF ni se activa analítica externa. Para la etapa de visitas, se propone separar por curso y por enlace de difusión (Classroom/colegas) mediante parámetros UTM. Una URL etiquetada identifica el canal de llegada, no a la persona: un colega puede reenviarla a su alumnado. No se inferirá uso pedagógico solo por un clic. Esta rectificación corrige el alcance de las decisiones anteriores y queda pendiente de publicación.
